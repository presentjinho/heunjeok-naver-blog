(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.HeunjeokPostLog=api})(typeof globalThis!=='undefined'?globalThis:this,function(){'use strict';
  ...
  function createEntry(input={},now=()=>Date.now()){
    ...
    return{ok:true,value:{
      ...
      platform:PLATFORMS.includes(input.platform)?input.platform:'naver',
      role,
  ...
  function normalizeEntry(raw){
    ...
    if(raw.metrics&&typeof raw.metrics==='object'){
      const views=toNonNegativeInt(raw.metrics.views);const inflow=toNonNegativeInt(raw.metrics.inflow);const impressions=toNonNegativeInt(raw.metrics.impressions);const likes=toNonNegativeInt(raw.metrics.likes);const bookmarks=toNonNegativeInt(raw.metrics.bookmarks);
      if([views,inflow,impressions,likes,bookmarks].some(v=>v!==null))metrics={views,inflow,impressions,likes,bookmarks,recordedAt:Number.isFinite(raw.metrics.recordedAt)?raw.metrics.recordedAt:Date.now()};
    }
    return{id:String(raw.id||randomId()),topic,postType:clean(raw.postType,20)||'',platform:PLATFORMS.includes(raw.platform)?raw.platform:'naver',role:ROLES.includes(raw.role)?raw.role:'기타',keywords:parseKeywords(raw.keywords),url:clean(raw.url,300),note:cleanMultiline(raw.note,500),publishedAt,metrics};
  }
  ...
  function updateMetrics(list,id,metrics={},now=()=>Date.now()){
    const views=toNonNegativeInt(metrics.views);const inflow=toNonNegativeInt(metrics.inflow);const impressions=toNonNegativeInt(metrics.impressions);const likes=toNonNegativeInt(metrics.likes);const bookmarks=toNonNegativeInt(metrics.bookmarks);
    return normalizeLog(list).map(entry=>{
      if(entry.id!==id)return entry;
      if([views,inflow,impressions,likes,bookmarks].every(v=>v===null))return{...entry,metrics:null};
      return{...entry,metrics:{views,inflow,impressions,likes,bookmarks,recordedAt:now()}};
    });
  }
  ...
  return{ROLES,PLATFORMS,FOLLOWUP_DAYS,createEntry,normalizeEntry,normalizeLog,addEntry,removeEntry,updateMetrics,dueForFollowup,summarize,parseKeywords,toNonNegativeInt};
});
