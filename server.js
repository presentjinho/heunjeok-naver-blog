...
const PUBLIC_FILES=new Set(['/index.html','/styles.css','/comments.css','/a11y.css','/connection.css','/core.js','/history.js','/photo-vault.js','/app.js']);
...
function validateRuntimeConfig(env=process.env){
  const errors=[];
  const origin=String(env.APP_ORIGIN||'http://localhost:3000');
  ...
  if(env.NODE_ENV==='production'&&String(env.SESSION_SECRET||'').length<32)errors.push('SESSION_SECRET must be at least 32 characters in production');
...
function createRateLimiter({limit=30,windowMs=60000}={}){const clients=new Map();return key=>{const now=Date.now();const current=clients.get(key);if(!current||current.resetAt<=now){clients.set(key,{count:1,resetAt:now+windowMs});return true}current.count+=1;return current.count<=limit}}
...
function parseCookies(value=''){return Object.fromEntries(value.split(';').map(part=>part.trim().split('=')).filter(parts=>parts.length===2).map(([key,val])=>[key,decodeURIComponent(val)]))}
...
function createOAuthStateManager({ttlMs=5*60*1000,now=()=>Date.now()}={}){
  const states=new Map();
  function issue(sessionId){const state=crypto.randomBytes(32).toString('base64url');states.set(crypto.createHash('sha256').update(state).digest('hex'),{sessionId,expiresAt:now()+ttlMs});return state}
...
function createSessionManager({ttlMs=30*60*1000,now=()=>Date.now()}={}){
  const sessions=new Map();
  function create(){const id=crypto.randomBytes(32).toString('base64url');const csrf=crypto.randomBytes(32).toString('base64url');sessions.set(id,{csrf,expiresAt:now()+ttlMs});return{id,csrf,ttlMs}}
...
function createAppServer(options={}){
  ...
  const defaultOrigins=['http://localhost:3000','http://127.0.0.1:3000'];const origins=new Set(options.allowedOrigins||(process.env.APP_ORIGIN?String(process.env.APP_ORIGIN).split(',').map(v=>v.trim()).filter(Boolean):defaultOrigins));
  ...
  const identityKey=String(options.identityKey||process.env.SESSION_SECRET||crypto.randomBytes(32).toString('hex'));
...
module.exports={PUBLIC_FILES,SECRET_NAMES,validateRuntimeConfig,securityHeaders,createRateLimiter,readJson,parseCookies,safeEqual,requestAllowed,createAuditLogger,createOAuthStateManager,createSessionManager,sessionId,sessionCookie,limitedJson,createNaverOAuthClient,createAppServer};
