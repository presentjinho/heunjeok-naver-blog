(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.HeunjeokCore=api})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function requiredDisclosure(relationship){return RELATIONSHIP_TEXT[relationship]||''}
  function hasRequiredDisclosure(text,relationship){const required=requiredDisclosure(relationship);return !required||String(text||'').trimStart().startsWith(required)}
  ...
  function analyzeTone(text){
    ...
    const endingCounts={soft:(value.match(/(?:해요|어요|아요|이에요|예요|네요|죠)(?=[.!?\s]|$)/g)||[]).length,formal:(value.match(/(?:합니다|입니다|습니다)(?=[.!?\s]|$)/g)||[]).length,casual:(value.match(/(?:했어|였어|됐어|갔어|왔어|봤어|했지)(?=[.!?\s]|$)/g)||[]).length};
    return{ending:friendly?'부드러운 존댓말':formal?'정중한 설명체':casual?'친근한 반말체':'차분한 서술형',sentenceLength:average<34?'짧고 빠른 문장':'여유 있는 문장',averageSentenceChars:average,endingCounts,paragraphStyle:paragraphs>=3?'문단을 자주 나눔':'문단을 길게 이어감',emoji:emoji?'이모지 사용':'이모지 거의 없음',connectors,sampleWords:countWords(value)};
  }
  function applyToneStrength(text,tone){const value=String(text||'');if(!tone||tone.strength!=='strong')return value;if(tone.ending==='정중한 설명체')return value.replace(/했어요(?=[.!?]|$)/g,'했습니다').replace(/였어요(?=[.!?]|$)/g,'였습니다').replace(/이에요(?=[.!?]|$)/g,'입니다');if(tone.ending==='부드러운 존댓말')return value.replace(/했습니다(?=[.!?]|$)/g,'했어요').replace(/였습니다(?=[.!?]|$)/g,'였어요').replace(/입니다(?=[.!?]|$)/g,'이에요');return value}
  ...
  function createHashtags(topic,category,memo,relationship,postType='visit'){
    ...
    const tags=[...words.slice(-1),(typeTags[postType]||typeTags.visit)[0],categoryTag[category]||categoryTag.local];
    if(relationship==='selfpaid'||/내돈내산|직접\s*구매|구매했/.test(String(memo||'')))tags.splice(2,1,'내돈내산');
    return[...new Set(tags)].slice(0,3).map(tag=>`#${tag}`);
  }
  ...
  function createPackage({topic,memo,tone,category='local',photoNotes='',relationship='none',postType='visit',experienceFields={}}){
    ...
    (plans[postType]||plans.visit).forEach(keys=>{
      const lines=keys.map(key=>byKey[key]).filter(Boolean).map(section=>applyToneStrength(polishEvidence(section.text,section.key,postType),tone));
